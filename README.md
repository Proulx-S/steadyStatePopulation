...
replacing itself. It's computed from the model's *unscaled* fertility shape (a triangular pulse,
before calibration — NOT what's plotted in the top-left panel, which shows the already-scaled
`fertility`) and the survivorship curve (the probability of surviving from birth to each age,
itself derived from the mortality curve): `R0 = sum(survivorship .* fertilityShape)`. Fertility is
then rescaled so this R0 hits `targetR0` (a `CONTROL PANEL` parameter in `doIt.m`, default `1`):
`fertility = fertilityShape * (targetR0 / R0)`. This calibration is what the whole model hinges on
